//@ts-nocheck
// No Typescript check is scary (otherwise Deno throws 30 errors ...)
// but ... I trust the library author
// https://github.com/euberdeveloper/dree patched
// to work with Deno (I only changed the imports)
import { nodeCrypto, nodeFs, nodePath, nodeUtil } from "../deps.ts";

const { resolve, basename, extname, relative } = nodePath;
...
function _parse(
  ...
  const lines = children.map((child, index) => {
    ...
    if (
      options.extensions && type === Type.FILE &&
  ...
  if (options.sorted) {
    children = children.sort((x, y) =>
      typeof options.sorted === "boolean"
        ...
        : options.sorted(x.relativePath, y.relativePath)
    );
  }
  children
...
export function parseTree(dirTree: Dree, options?: ParseOptions): string {
  ...
  result +=
    (dirTree.children ? _parseTree(dirTree.children, "\n ", opt, 1) : "");
  return result;
}
